...
var instantiateUpdgradeChaincode = async function(peers, channelName, chaincodeName, chaincodeVersion, functionName, chaincodeType, args, username, org_name) {
	...
	try {
		...
		const endorsement_policy = {
			identities: [
					{ role: { name: 'member', mspId: 'VZOrgMSP' } },
					{ role: { name: 'member', mspId: 'CTLOrgMSP' } }
				],
					policy: {
		...
		results = await channel.sendInstantiateProposal(request, 150000); //instantiate takes much longer
