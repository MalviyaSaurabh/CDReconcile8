...
//logger.info('one');
		if(client.status && client.status !=200){
			return client ;
		} 
...
//logger.info('four');
		...
		if (response_payloads) {
			for (let i = 0; i < response_payloads.length; i++) {
				...
				let firstItem = response_payloads[i].toString('utf8');
				if (firstItem.indexOf("Error:") > -1) {
					logger.info("First Item is String: " + firstItem);
					let response = {status :501 , message : firstItem};
                			return response;	
				}
				else {
		...
		let response = {status :501 , message : error.toString()};
		return response;
	}
};
