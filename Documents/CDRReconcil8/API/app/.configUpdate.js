...
var configUpdate = async function (channelName, username, orgName, crl) {
	...
	try {
		...
		const configJson = await agent.post('http://127.0.0.1:14033/protolator/decode/common.Config', origConfigProto).buffer();
		...
		const updatedConfigPb = await agent.post('http://127.0.0.1:14033/protolator/encode/common.Config', JSON.stringify(updatedConfigJson)).buffer();
		const formData = {
			channel: channelName,
			original: {
		...
		var configProto = await new Promise((resolve, reject) => {
			requester.post({
				url: 'http://127.0.0.1:14033/configtxlator/compute/update-from-configs',
				encoding: null,
				headers: {
					accept: '/',
					expect: '100-continue'
